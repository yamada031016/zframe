...
export fn wasmAnalyze(top: u32, len: u32) u8 {
    ...
    if (Wasm.analyzeSection(.Type)) |typeInfo| {
        MEMORY[top + offset] = @intCast(typeInfo.len);
        offset += 1;
        for (typeInfo) |info| {
            const serialized_info = info.serialize() catch unreachable;
            @memcpy(MEMORY[top + offset ..], serialized_info);
    ...
    // } else |_| {}
    return @intCast(offset - data.len);
}
