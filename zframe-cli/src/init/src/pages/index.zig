...
pub fn index() node.Node {
    ...
    return main.setClass("bg-gradient-to-br from-gray-900 via-gray-800 to-gray-700 text-white min-h-screen flex flex-col").init(.{
        Head("zframe", .{}),
        section.setClass("flex flex-col items-center justify-center text-center flex-grow px-4 py-20 space-y-8").init(.{
            h1
                .setClass("text-6xl font-extrabold text-[#4fd1c5] drop-shadow-md")
                .init("Build the Future with zFrame"),
            p.setClass("text-lg text-gray-300 max-w-2xl").init(.{
            ...
            a
                .setClass("px-6 py-3 bg-[#3182ce] text-white font-semibold rounded-lg shadow-lg hover:bg-[#2b6cb0] hover:shadow-2xl transition-all")
                .init(.{
